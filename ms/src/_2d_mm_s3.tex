...
In addition to spatial data, temporal changes in ant presence were analyzed to examine the relationship between the number of ants and time during the study period. For example, in subset \textit{B03}, where two bait types, sucrose (labeled ‘S’) and peptone (labeled ‘P’), were placed in the same image, the aim was to understand the ants’ foraging preferences. If the sucrose bait was positioned in the upper-right and the peptone in the lower-left of the image, a simple linear function (Equation \ref{eq3}) was used to separate the ants based on their attraction to the respective baits:
