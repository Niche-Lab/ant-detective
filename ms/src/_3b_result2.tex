...
\begin{figure}[H]
    ...
    \caption{Comparison of detection results between the original image and SAHI-aid inference results. The SAHI-aid inference was conducted by YOLO11n with parameters of divider number = 4 and overlap ratio = 0. (a) The original scale (b) Three examples in rows showing zoomed-in views of the original image. The detected ants are highlighted with blue bounding boxes.}
    \label{fig:s2_vis}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{Model performance on the \textit{Dense} subset with different models, calibration strategies, and search approaches. The top panel shows the mAP@0.5 performance of YOLO11n, YOLO11m, and RT-DETR-L models under different strategies and search approaches. The letter in black indicates the Tukey HSD test result, where different letters indicate significant differences between search approaches. While the letter in white font and purple background indicates the Tukey HSD test result between calibration strategies with grid search as the search approach of optimal parameters. The bottom panel compares the computational cost of different search approaches in terms of hyperparameter tuning time. The black font indicates the Tukey HSD test results between search approaches.}
...
\begin{table}[H]
    \centering
    \caption{Model performance comparison on the subset \textit{Dense} across different calibration strategies and search approach. FT: finetuning with target images, unsup.: unsupervised SAHI tuning, sup.: supervised SAHI tuning. BO: Bayesian optimization.}
...
Although this represents a relatively small search space, it effectively demonstrates how exploration-exploitation trade-offs apply to hyperparameter tuning processes. Exploration shows greater likelihood of discovering optimal parameter choices, though at higher computational cost compared to grid search.
...
\begin{table}[H]
    \centering
    \caption{Performance of SAHI inference on the \textit{Dense} subset with different hyperparameter settings. The performance is evaluated in terms of mAP@0.5 and the frames per second (FPS) by NVIDIA L40S GPU. The largest numbers of mAP and FPS are highlighted in bold. The frequency of each hyperparameter setting determined by the search approach is also shown.}
    \label{tab:s2_hpmap}
    \begin{tabular}{ccccccc}
        \midrule
    \textbf{Param. 1:} & \textbf{Param. 2:} & \textbf{Grid Search} & \textbf{Exploration BO} & \textbf{Exploitation BO} & \textbf{Exp.} & \textbf{Frames} \\
    \textbf{Divider} & \textbf{Overlap} & \textbf{Freq.} & \textbf{Freq.} & \textbf{Freq.} & \textbf{mAP@0.5} & \textbf{per Sec.} \\
    \midrule
        4 & 0 & 100.0\% & 41.7\% & 30.0\% & \textbf{81.8\%} & 2.26 \\
...
Both counting methodologies revealed consistent trends in foraging behavior, with SINV-3 infected ants exhibiting consistently elevated foraging activity compared to uninfected controls. This finding demonstrates the CV system's capability to accurately capture ant foraging dynamics in a high-throughput manner, providing a powerful tool for investigating pathogen effects on foraging patterns and potentially other complex social behaviors.
The successful validation of automated counting against manual methods, combined with the system's ability to detect biological phenomena of interest, establishes its utility for large-scale behavioral ecology studies where manual counting would be prohibitively time-consuming or prone to observer bias.
